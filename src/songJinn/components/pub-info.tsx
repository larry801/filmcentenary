...
export const PubInfo = ({G, ctx}: IPubInfo) => {
    ...
    return <Grid container><Grid item><Paper>
        ...
        <div><label>弃牌：{s.discard.map(p => getCardById(p).name)}</label></div>
        <div><label>移除：{s.remove.map(p => getCardById(p).name)}</label></div>
        <div><label>手牌数：</label></div>
        <div><label>发展牌：{s.remove.map(p => getCardById(p).name)}</label></div>
        ...
        <Grid item><Paper><label>金</label>
            ...
            {/* TODO 手牌数 弃牌 移除 */}
            <div><label>弃牌：{j.discard.map(p => getCardById(p).name)}</label></div>
            <div><label>移除：{j.remove.map(p => getCardById(p).name)}</label></div>
            <div><label>手牌数：</label></div>
            <div><label>发展牌：{j.develop.map(p => getCardById(p).name)}</label></div>
            {G.turn > 6 && <div><label>绍兴和议分数：</label></div>}
        </Paper></Grid>
    </Grid>
}